        	...
        	$("#edit-user-btn").click(function(){
        		...
        		$.ajax({
		            ...
		            data: {
		                ...
		                total_period: $("input[name=total_period]").val(),
		                theoretical_period: $("input[name=theoretical_period]").val(),
		                experimental_period: $("input[name=experimental_period]").val()
		            },
		            dataType : 'json',
		            success : function(result) {
		                if(result.success){
							$("#edit-user-result").show();
		                     $("#edit-user-result div div").html("编辑成功");
		                }else{
							$("#edit-user-result").show();
		                     $("#edit-user-result  div div").html("编辑资料失败，请重试");
		                }
		            },
		            error : function() {
		            	$("#edit-user-result").show();
			...
			$("#add-lessonplan-btn").click(function(){
        		...
        		$.ajax({
		            ...
		            data: {
		                ...
		                total_week: $("input[name=total_week]").val(),
	                ...
	                if(result.success){
	                	...
	                	if(!result.data){
							...
							$.each(result.data.courses, function(index,item) {
								console.log("==================================", index)
								$('.courseMessage').append('<tr><td>' + item.id + '</td><td>'+item.coursename+'</td><td>'+item.total_period+'</td><td>'+item.theoretical_period+'</td><td>'+item.experimental_period+'</td><td><button type="submit" class="btn btn-primary" id="btnModify">修改</button><button id="btnChgDIStatus" class="btn btn-danger" >删除</button></td></tr>')
							})
							...
							t.on('click', 'button#btnModify', function () {
								...
								var data = {"course":{"coursename":dataTab[1],"total_period":dataTab[2],"theoretical_period":dataTab[3],"experimental_period":dataTab[4]}};
								// console.log(data);
	                ...
	                if(result.success){
	                	...
	                	if(!result.data){
							...
							$.each(result.data.lessonplan, function(index,item) {
								console.log("==================================", index)
								$('.courseMessage').append('<tr><td>' + item.id + '</td><td>'+item.semester+'</td><td>'+item.coursename+'</td><td>'+item.teachername+'</td><td>'+item.classname+'</td><td><button type="submit" class="btn btn-primary" onclick="javaScript:window.location.href=\'generalles.html\'">修改</button><button type="submit" class="btn btn-danger" onclick="deleteLes(\''+item.id+'\')">删除</button></td></tr>')
							})
	                	}
	                }else{
	                    $("#topics-body").html('<div class="alert alert-info" role="alert">'+result.msg+'</div>');
